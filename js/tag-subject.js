...
//   section.textContent = value1;
//   subject.textContent = value2;
//   if (value2 == null) {
...
async function getSubject() {
  ...
  var value2 = url.searchParams.get('subject');
  // var sectionName = '';
  var dataCourse = await getDataFromJSON('./data/courses.JSON');
  var dataCourseList = await getDataFromJSON('./data/coursesList.JSON');
  var idSection;
  ...
  dataCourseList.forEach((item) => {
    item.course.forEach((ele) => {
      if (coverStringToFormat(ele) == value2) {
        subject.textContent = ele;
        searchWrapperTitle.textContent = `${ele} courses`;
        subject.href = `./tag-subject.html?section=${value1}&subject=${value2}`;
      }
    });
  });
...
async function getSubjectBaseSubject() {
  var value = await getSubject();
...
// http://127.0.0.1:5500/tag-section.html?section=kinh-doanh-va-khoi-nghiep&subject=chung-khoan
...
async function renderDefaultSubject() {
  var arr = await getSubjectBaseSubject();
...
async function handleClickSort(value, index) {
  let arr = await getSubjectBaseSubject();
...
async function handleSortStar(item, value) {
  let arr = [];
  if (item.checked == true) {
    let data = await getSubjectBaseSubject();
    data.forEach((ele) => {
      if (ele.star >= value) {
        arr.push(ele);
      }
...
async function getSectionLists() {
  var idSection;
  var arr = [];
  var sectionValue = await getSection();
